...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-screen w-full flex justify-center items-center overflow-hidden bg-black px-6 md:px-12 py-20 md:py-32">
      ...
      {/* FIX: Added mt-16 on mobile to ensure it clears the new dynamic header, keeping it centered */}
      <div className="relative flex flex-col md:flex-row items-center justify-between w-full max-w-[1200px] z-10 gap-4 md:gap-16 pointer-events-none mt-16 md:mt-0">
        ...
        <div className="flex flex-col space-y-6 md:space-y-8 pointer-events-auto w-full z-20">
          <div className="flex flex-col">
            <motion.h1 
              ...
              {/* FIX: Using 13vw on mobile forces the text to act like a perfectly sized billboard without breaking words */}
              className="text-[13vw] sm:text-[9vw] md:text-[clamp(3.5rem,8vw,6rem)] font-bold tracking-tighter leading-[0.85] uppercase text-white"
            >
              Stop<br />
            ...
            <motion.h1 
              ...
              className="text-[13vw] sm:text-[9vw] md:text-[clamp(3.5rem,8vw,6rem)] font-bold tracking-tighter leading-[0.85] uppercase text-white mt-1 md:mt-2"
            >
              Start<br />
          ...
          <motion.p 
            ...
            {/* FIX: Tightened the max-width on mobile to keep the paragraph looking like a neat block */}
        ...
        <motion.div 
          style={{ y: yParallax }} 
          {/* FIX: -mt-8 on mobile pulls the icon up into the composition so it doesn't look like an afterthought floating at the bottom */}
          className="flex-1 flex justify-center md:justify-end w-full relative z-10 -mt-8 md:mt-0"
        >
          <img 
            src="/kahory media icon png 3-d.png" 
            alt="Kahory Icon" 
            {/* FIX: 75% max width on mobile keeps it large and imposing, but structured */}
            className="w-full max-w-[75%] sm:max-w-[65%] md:max-w-[80%] lg:max-w-[800px] h-auto drop-shadow-[0_0_150px_rgba(74,14,27,0.4)] select-none pointer-events-none"
            draggable="false"
          />
        </motion.div>
        ...
        <motion.div 
          ...
          {/* FIX: Moved to top-0 right-0 on mobile to balance the left-heavy text. Stays bottom-left on desktop! */}
          className="absolute top-0 right-0 md:-bottom-24 md:left-0 md:top-auto w-16 h-16 md:w-20 md:h-20 flex items-center justify-center opacity-60 z-20 pointer-events-none"
        >
